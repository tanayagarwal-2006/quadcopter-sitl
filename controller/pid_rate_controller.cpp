...
vector3 rate_controller::update(const vector3& desired_rates, const vector3& measured_rates){
    ...
    if(integral_windup_prevent(roll_error,theoretical_roll_torque,temp_max_axis_torque)){
    ...
    if(integral_windup_prevent(pitch_error, theoretical_pitch_torque, temp_max_axis_torque)){
    ...
    if (integral_windup_prevent(yaw_error, theoretical_yaw_torque, temp_max_axis_torque)) {
    ...
    torque_command.x = constrain_to_value(torque_command.x, -temp_max_axis_torque, temp_max_axis_torque);
    torque_command.y = constrain_to_value(torque_command.y, -temp_max_axis_torque, temp_max_axis_torque);
    torque_command.z = constrain_to_value(torque_command.z, -temp_max_axis_torque, temp_max_axis_torque);\
    //Normalize     
    torque_command.x /= temp_max_axis_torque;
    torque_command.y /= temp_max_axis_torque;
    torque_command.z /= temp_max_axis_torque;
