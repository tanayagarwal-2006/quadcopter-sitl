...
udp_bridge::udp_bridge(const std::string& target_ip, int send_port, int recv_port) {
    ...
    // Setup Send Socket (Talking to MATLAB)
    ...
    matlab_addr.sin_port = htons(send_port);
    inet_pton(AF_INET, target_ip.c_str(), &matlab_addr.sin_addr);

    // Setup Receive Socket (Listening to MATLAB)
...
void udp_bridge::send_motors(const motor_payload& motors) {
    sendto(send_socket, (const char*)&motors, sizeof(motor_payload), 0, 
           (sockaddr*)&matlab_addr, sizeof(matlab_addr));
}
